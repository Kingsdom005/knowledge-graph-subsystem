...
def parse_ternary(choice):
    ...
    # parse into triples
    for item in data:
        id_triples.append([item['id'], "name", item['title']])
        dated_triples.append([item['id'], "dated", item['dated']])
        artist_triples.append([item['id'], "artist", item['artist']])
        role_triples.append([item['id'], "role", item['role']])
        department_triples.append([item['id'],"department",item['department']])
        medium_triples.append([item['id'], 'medium', item['medium']])
        description_triples.append([item['id'], "description", item['description']])
        comments_triples.append([item['id'], "comments", item['comments']])
        web_url_triples.append([item['id'], "web_url", item['web_url']])
        img_url_triples.append([item['id'], "img_url", item['img_url']])
...
if __name__ == "__main__":
    ...
    fields = ['name', 'dated', 'artist', 'role', 'department', 'medium', 'description', 'comments', 'web_url', 'img_url']
    file_names = ['name', 'dated', 'artist', 'role', 'department', 'medium', 'description', 'comments', 'web_url','img_url']

    # double loop to write data into tables
    for choice in choices:
        data = list(parse_ternary(choice=choice))
        for i in range(len(data)):
            xw_toExcel(data=data[i], fileName="triples/f{}/node_".format(str(choice))+file_names[i]+".csv", title=['id', 'relation', fields[i]])
